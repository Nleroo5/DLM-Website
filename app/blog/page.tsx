...
export default function BlogPage() {
  ...
  return (
    <main className="blog-page min-h-screen bg-[#0B1D2E] text-[#F8F6F3]">
      {/* Hero Section */}
      <section className="pt-[180px] pb-[80px] px-6">
        <div className="max-w-[900px] mx-auto text-center">
          ...
          <p className="text-[#D4A574] text-[clamp(1.2rem,2.5vw,1.6rem)] max-w-[700px] mx-auto leading-relaxed">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {posts.map((post) => (
              <Link
                key={post.slug}
                href={`/blog/${post.slug}`}
                className="group bg-gradient-to-br from-[#0B1D2E] to-[#162E42] border border-[rgba(95,169,159,0.2)] rounded-[24px] p-8 hover:border-[#5FA99F] transition-all duration-300 hover:-translate-y-1"
              >
                {/* Category Tag */}
                <div className="inline-block bg-[rgba(212,165,116,0.2)] text-[#D4A574] px-3 py-1 rounded-full text-[0.875rem] font-medium mb-4">
                  {post.category}
                </div>

                {/* Title */}
                <h2 className="text-[#F8F6F3] font-serif text-[1.5rem] font-normal leading-tight mb-3 group-hover:text-[#5FA99F] transition-colors">
                  {post.title}
                </h2>

                {/* Excerpt */}
                <p className="text-[#F8F6F3] opacity-80 text-[1rem] leading-relaxed mb-4">
                  {post.excerpt}
                </p>

                {/* Meta Info */}
                <div className="flex items-center justify-between text-[0.875rem] text-[#D4A574]">
                  <span>{post.date}</span>
                ...
                <div className="mt-4 pt-4 border-t border-[rgba(95,169,159,0.2)]">
                  <p className="text-[0.875rem] text-[#F8F6F3] opacity-70">
                    By {post.author}
                  </p>
                </div>
              </Link>
      ...
      <section className="pb-[100px] px-6">
        <div className="max-w-[700px] mx-auto bg-gradient-to-br from-[#5FA99F] to-[#4A8B82] rounded-[24px] p-10 text-center">
          <h2 className="text-[#0B1D2E] font-serif text-[2rem] font-normal mb-4">
            Get Meta Ads Insights Delivered
          </h2>
          <p className="text-[#0B1D2E] opacity-90 text-[1.1rem] mb-6">
